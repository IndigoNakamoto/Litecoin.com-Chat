...
async def fake_astream_query_with_chart(_query, _history):
    yield {
        "type": "chart_spec",
        "artifact": {
            "type": "chart",
    ...
    yield {"type": "chunk", "content": "Here is a chart."}
    yield {"type": "complete", "from_cache": False}


def test_chat_stream_endpoint_emits_chart_spec_sse_event(client, monkeypatch):
    import backend.main as main_module

    class FakePipeline:
        astream_query = staticmethod(fake_astream_query_with_chart)
    ...
    headers = {"X-Fingerprint": "fp:testchallenge:testhash"}
    payload = {"query": "chart realized price", "chat_history": []}
    ...
    statuses = [event["status"] for event in sse_payloads]
    assert "chart_spec" in statuses
    assert statuses.index("chart_spec") < statuses.index("streaming")
    chart_event = next(event for event in sse_payloads if event["status"] == "chart_spec")
    assert chart_event["artifact"]["chart"]["series"][0]["path"] == "realized-price"
